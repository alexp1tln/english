...
export default function IrregularVerbs({ setView }: { key?: string, setView: (v: ViewState) => void }) {
  ...
  const generateOptions = (item: {verb: IrregularVerb, type: 'past'|'participle'|'translation'}) => {
    ...
    if (type === 'translation') {
      ...
      let regForm = verb.base;
      if (regForm.endsWith('e')) regForm += 'd';
      else if (regForm.endsWith('y')) regForm = regForm.slice(0, -1) + 'ied';
      else if (regForm.match(/[bcdfghjklmnpqrstvwxyz]$/)) regForm += 'ed';
      else regForm += 'ed';
      opts.add(regForm);
      ...
      while(opts.size < 4) {
         ...
         const opt = type === 'past' ? randomVerb.past.split('/')[0] : randomVerb.participle.split('/')[0];
         opts.add(opt);
      }
    }
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col h-full absolute inset-0 w-full bg-[#000000]">
            ...
            >
              ...
              <div className="flex flex-col gap-4">
                {quizOptions.map((opt, idx) => {
                  ...
                  let btnClass = "p-5 rounded-[2.5rem] border text-center text-lg transition-all duration-300 relative overflow-hidden ";
...
fs.writeFileSync('src/components/IrregularVerbs.tsx', code);
